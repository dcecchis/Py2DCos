...
def reader(filename):
    ...
    if filename[1] == "xlsx":
        ...
        row = int(row)
        skipRows = row-1
        spec1 = pd.read_excel(filename[0], header=None, index_col=0, sheet_name=sheet, usecols=column, skiprows=skipRows)

        if filename[-1] == True: # check if columns are labeled
            firstCol = column.split(':')[0] # first column of the column range
            nextCol = chr(ord(firstCol)+1) # get the following column, because the heading of the wavenumber column isnt relevant
            colRange = nextCol + column[1:]
            labels = pd.read_excel(filename[0], header=None, sheet_name=sheet, usecols=colRange, skiprows=skipRows-1, nrows=1).values[0]
...
class twoDspecies:
    ...
    def plotFunction(self, corrType='homo', calcMethod='HT', refSpectra='ini', colorMap='coolwarm',
                     numOfContour=6, syncDiag='main', asyncDiag='anti', xAxis='decreasing', colorMapIntensity=1.0,
                     colorLines='black', colorLinesIntensity=0.6, shownGraph='both', canvas = False, figure = None, eqSpaced = True):
        figure.clear()
        ...
        if shownGraph == 'sync':
            ...
            panels['central'].contour(num_wave, num_wave2, data, numOfContour, cmap=None, vmin=zmin, vmax=zmax,
            ...
            panels['right'].contour(num_wave, num_wave2, data, numOfContour, cmap=None, vmin=zmin, vmax=zmax,
                                      colors=colorLines, alpha=colorLinesIntensity, linewidths=linewidth)
