...
class Async2DPlot(PlotBase):

    def draw(self, model: "CorrelationModel", settings: PlotSettings) -> plt.Figure:
        ...
        elif settings.peaks is Peaks.NEGATIVE:
            data = np.where(data < 0, data, np.nan)

        zmin, zmax = np.nanmin(data), np.nanmax(data)

        # asynchronous 2D correlation plot CHECK ORDER
        ...
        caxA = plt.axes(fig.add_subplot(gs[1, 4]))
